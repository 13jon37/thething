...
pub struct TileManager<'a> {
    level: [[u8; LEVEL_HEIGHT]; LEVEL_WIDTH],
    tile_set: Texture<'a>,
}
...
impl<'a> TileManager<'a> {
    pub fn new<T>(creator:&'a TextureCreator<T>) -> Self {
        Self {
            level: [[TILE_EMPTY, TILE_EMPTY, TILE_EMPTY, TILE_EMPTY, TILE_EMPTY],
            ...
            tile_set: load_texture("Assets/grass.png",
                      &creator),
        }
    }
    // Tile method needs tile coords too  
    fn render_tile(&self, canvas: &mut WindowCanvas, coordinates: Point<i32>) -> Result<(), String> {
        let src_rect = Rect::new(0, 0, TILE_SIZE, TILE_SIZE);
        let dst_rect = Rect::new(coordinates.x as i32 * src_rect.w,
